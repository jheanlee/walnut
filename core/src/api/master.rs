...
use crate::orm::master::{
    db_authenticate_master, db_delete_master, db_is_present, db_list_master, db_modify_master,
};
...
pub async fn signup_availability_middleware(
    request: Request,
    next: Next,
...
pub async fn is_username_available(
    Query(query): Query<IsUsernameAvailableQuery>,
...
pub async fn master_signup(
    Json(master_signup): Json<MasterSignup>,
) -> Result<impl IntoResponse, ApiError> {
...
pub async fn new_master(
    Json(master_modification): Json<MasterModification>,
) -> Result<impl IntoResponse, ApiError> {
    let is_present = db_is_present(master_modification.username.clone()).await?;
    if !is_present {
        db_modify_master(
            master_modification.username,
            master_modification.password,
            master_modification.admin,
        )
        .await?;
        Ok(StatusCode::OK)
    } else {
...
pub async fn delete_master(
    Json(master_deletion): Json<MasterDeletion>,
) -> Result<impl IntoResponse, ApiError> {
...
pub async fn master_login(
    Json(master_login): Json<MasterLogin>,
    ...
    let response_builder =
        Response::builder().header(http::header::CONTENT_TYPE, "application/json");
    Ok(match res {
        (StatusCode::OK, id) => response_builder.body(Body::from(
            json!(
              {
